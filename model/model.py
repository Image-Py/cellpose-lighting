...
class CellPosenet(BaseModel):
    """The input to the neural network was a two-channel image with the primary channel
    corresponding to the cytoplasmic label, and the optional secondary channel correspoinding
    to nuclei, which in all case was a DAPI stain.

    When a second channel was not available, it was replaced with an image of zeros. Raw pixel
    intensities were scaled for each image so that the 1 and 99 percenitiles correspoinded to 0 and 1
